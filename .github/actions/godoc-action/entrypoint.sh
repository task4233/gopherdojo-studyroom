...
if [ "$2" = "" ]; then
    WORKING_DIR="kadai2/task4233/eimg"
fi
...
generate_godoc() {
    echo "url: http://localhost:8080/pkg/github.com/task4233/gopherdojo-studyroom/${WORKING_DIR}"
    godoc -url "http://localhost:8080/pkg/github.com/task4233/gopherdojo-studyroom/${WORKING_DIR}" | sed -e '1d' | sed 's/\/lib\/godoc/\./g' > ./${DOCNAME}/index.html
}
